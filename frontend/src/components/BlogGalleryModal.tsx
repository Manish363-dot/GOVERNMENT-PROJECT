...
const ALL_GALLERY_IMAGES = [
    ...
    '/assets/gallery/WhatsApp Image 2026-09-14 at 6.40.55 PM copy 2.jpeg',
    '/assets/gallery/WhatsApp Image 2026-09-14 at 6.40.55 PM copy.jpeg',
    ...
    '/assets/gallery/WhatsApp Image 2026-09-14 at 6.43.56 PM (2).jpeg',
    '/assets/gallery/WhatsApp Image 2026-09-14 at 6.43.56 PM.jpeg',
    // '/assets/gallery/WhatsApp Image 2026-09-14 at 6.43.58 PM (1).jpeg',
    // '/assets/gallery/WhatsApp Image 2026-09-14 at 6.43.58 PM.jpeg',
    '/assets/gallery/WhatsApp Image 2026-09-14 at 6.44.00 PM.jpeg',
    '/assets/gallery/WhatsApp Image 2026-09-14 at 6.44.01 PM.jpeg',
...
export function BlogGalleryModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            if (e.key === 'ArrowRight') setCurrentIndex(prev => (prev + 1) % PLACEHOLDER_IMAGES.length);
            if (e.key === 'ArrowLeft') setCurrentIndex(prev => (prev - 1 + PLACEHOLDER_IMAGES.length) % PLACEHOLDER_IMAGES.length);
    ...
    const nextImage = () => setCurrentIndex((prev) => (prev + 1) % PLACEHOLDER_IMAGES.length);
    const prevImage = () =>
        setCurrentIndex((prev) => (prev - 1 + PLACEHOLDER_IMAGES.length) % PLACEHOLDER_IMAGES.length);

    return (
        <div className="fixed inset-0 z-[9999] bg-black/70 backdrop-blur-sm flex flex-col animate-in fade-in duration-300">
            {/* Transparent background image without white washout */}
            <div
            ...
            <header className="relative z-20 shrink-0 bg-white border-b border-slate-200 shadow-xs">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between h-16 sm:h-20">
                        ...
                        {/* Close button — takes place of Sign-In */}
                        <Button
                            variant="outline"
                            size="sm"
                            onClick={onClose}
            ...
            {/* Scrollable Content Area — project bg with hardware acceleration */}
            <div id="modal-scroll-area" className="relative z-10 px-4 sm:px-8 py-6 flex flex-col overflow-y-auto overflow-x-hidden flex-1 scroll-smooth transform-gpu">

                {/* Carousel Area */}
                <div className="relative w-full max-w-7xl mx-auto flex items-center justify-center select-none py-6 pb-12">

                    {/* Left Tilted Image (Previous) */}
                    <div className="hidden sm:block absolute left-0 md:left-8 top-1/2 -translate-y-1/2 w-[25%] md:w-[22%] aspect-[4/3] z-0 opacity-[0.65] shadow-lg border border-slate-200/50 p-1.5 bg-white -rotate-[3deg] scale-[0.85] transition-all duration-700 pointer-events-none rounded-sm blur-[1px]">
                        <img
                            loading="lazy"
                            src={PLACEHOLDER_IMAGES[(currentIndex - 1 + PLACEHOLDER_IMAGES.length) % PLACEHOLDER_IMAGES.length]}
                            alt=""
                    ...
                    <div className="hidden sm:block absolute right-0 md:right-8 top-1/2 -translate-y-1/2 w-[25%] md:w-[22%] aspect-[4/3] z-0 opacity-[0.65] shadow-lg border border-slate-200/50 p-1.5 bg-white rotate-[3deg] scale-[0.85] transition-all duration-700 pointer-events-none rounded-sm blur-[1px]">
                        <img
                            loading="lazy"
                            src={PLACEHOLDER_IMAGES[(currentIndex + 1) % PLACEHOLDER_IMAGES.length]}
                            alt=""
                            className="w-full h-full object-cover rounded-sm grayscale-[15%]"
                        />
                    </div>
                    ...
                    <button
                        ...
                        className="absolute left-1 sm:left-1/4 md:left-[21%] z-20 p-2 sm:p-3 bg-white hover:bg-slate-50 hover:scale-105 text-navy-900 rounded-full shadow-[0_4px_14px_0_rgba(0,0,0,0.12)] border border-slate-200 transition-all focus:outline-none"
                    >
                    ...
                    <div className="relative z-10 w-full sm:w-[65%] md:w-[50%] lg:w-[42%] flex flex-col items-center group">
                        ...
                        {/* Subtle offset shadow/depth layer */}
                        <div className="absolute inset-0 bg-black/[0.04] translate-y-3 blur-xl rounded-md transition-all duration-500"></div>

                        {/* Main Image Polaroid */}
                        <div className="relative w-full aspect-[4/3] p-1.5 sm:p-2 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-slate-200/80 rounded-md transition-transform duration-500 will-change-transform">
                            <img
                                loading="lazy"
                                src={PLACEHOLDER_IMAGES[currentIndex]}
                                alt={`Gallery featured image ${currentIndex + 1}`}
                    ...
                    <button
                        ...
                        className="absolute right-1 sm:right-1/4 md:right-[21%] z-20 p-2 sm:p-3 bg-white hover:bg-slate-50 hover:scale-105 text-navy-900 rounded-full shadow-[0_4px_14px_0_rgba(0,0,0,0.12)] border border-slate-200 transition-all focus:outline-none"
                    >
                ...
                {/* Editorial Caption Area — placed outside the carousel so it never overlaps */}
                <div className="w-full max-w-3xl mx-auto flex flex-col items-center justify-center text-center px-4 mt-8 sm:mt-10 mb-6 z-10 relative">
                ...
                {/* ── Official Government Quote & Ministers Banner (High-Fidelity Navy Theme) ── */}
                <div className="w-full max-w-7xl mx-auto px-2 sm:px-6 mt-4 mb-10">
                    {/* Outer Luxury Navy Government Container */}
                    <div className="w-full p-4 sm:p-6 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.7)] border border-amber-500/40 relative overflow-hidden backdrop-blur-2xl" style={{ background: 'radial-gradient(circle at 15% 15%, #0e274c 0%, #08162e 60%, #040d1c 100%)' }}>

                        {/* Top Tricolor Accent Line */}
                        <div className="absolute top-0 left-0 right-0 h-1.5 shadow-sm" style={{ background: 'linear-gradient(90deg, #FF9933 0%, #FFFFFF 50%, #138808 100%)' }}></div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch mt-1 relative z-10">
                            ...
                            <div className="lg:col-span-8 rounded-2xl p-5 sm:p-6 flex flex-col sm:flex-row items-center gap-6 border border-white/10 shadow-[0_10px_30px_rgba(0,0,0,0.4)] relative overflow-hidden group hover:border-amber-400/40 transition-all duration-300" style={{ background: 'linear-gradient(145deg, rgba(15, 33, 64, 0.85) 0%, rgba(7, 18, 38, 0.95) 100%)' }}>

                                {/* PM Photo Frame with Dual Gold Ring & Soft Glow */}
                                ...
                                <div className="flex-1 flex flex-col justify-between text-center sm:text-left text-white h-full py-1">
                                    <div>
                                        ...
                                        <p className="text-slate-100 text-base sm:text-lg md:text-xl font-semibold leading-relaxed tracking-wide drop-shadow-md font-poppins">
                                            “The goal of the <span className="text-amber-300 font-bold underline decoration-amber-400/40 underline-offset-4">'Swachh Bharat Mission'</span> is to make a garbage-free city, a city completely free of garbage.”
                                        </p>
                                    </div>
                                    ...
                                    <div className="mt-5 border-t border-white/10 pt-3.5 flex flex-col sm:flex-row items-center sm:items-end justify-between gap-2">
                                        <div>
                                            <p className="text-amber-400 font-bold text-base sm:text-lg tracking-wide drop-shadow">Shri Narendra Modi</p>
                                            <p className="text-slate-300 text-xs font-semibold tracking-wider uppercase mt-0.5">Hon'ble Prime Minister of India</p>
                                        </div>

                                        {/* Hindi Signature graphic */}
                                        <div className="opacity-90 hover:opacity-100 transition-opacity">
                                            <span className="text-amber-300/80 text-xl font-extrabold italic tracking-widest select-none drop-shadow" style={{ fontFamily: 'Georgia, serif' }}>
                                                नरेन्द्र मोदी
                                            </span>
                                        </div>
                                    </div>
                            ...
                            <div className="lg:col-span-2 rounded-2xl p-4 flex flex-col items-center justify-center text-center border border-white/10 shadow-[0_10px_30px_rgba(0,0,0,0.4)] relative overflow-hidden group hover:border-amber-400/40 hover:-translate-y-1 transition-all duration-300" style={{ background: 'linear-gradient(145deg, rgba(15, 33, 64, 0.85) 0%, rgba(7, 18, 38, 0.95) 100%)' }}>
                                ...
                                <p className="text-white font-extrabold text-xs sm:text-sm leading-tight group-hover:text-amber-300 transition-colors">Shri Pushkar Singh Dhami</p>
                                <p className="text-amber-400 text-[10px] sm:text-xs font-semibold mt-1">Hon'ble Chief Minister</p>
                            ...
                            <div className="lg:col-span-2 rounded-2xl p-4 flex flex-col items-center justify-center text-center border border-white/10 shadow-[0_10px_30px_rgba(0,0,0,0.4)] relative overflow-hidden group hover:border-amber-400/40 hover:-translate-y-1 transition-all duration-300" style={{ background: 'linear-gradient(145deg, rgba(15, 33, 64, 0.85) 0%, rgba(7, 18, 38, 0.95) 100%)' }}>
                                ...
                                <p className="text-white font-extrabold text-xs sm:text-sm leading-tight group-hover:text-amber-300 transition-colors">Shri Satpal Maharaj</p>
                                <p className="text-amber-400 text-[10px] sm:text-xs font-semibold mt-1">Hon'ble Cabinet Minister</p>
                                <span className="inline-block px-2.5 py-0.5 rounded-full bg-white/5 border border-white/10 text-slate-300 text-[9px] font-semibold mt-1">
                                    Uttarakhand
                                </span>
                            </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5">
                        {ALL_GALLERY_IMAGES.map((img, idx) => (
                            <div
                                ...
                                className="relative aspect-[4/3] rounded-2xl overflow-hidden bg-navy-950/80 border border-white/15 shadow-[0_8px_25px_rgba(0,0,0,0.4)] hover:border-amber-400/50 hover:shadow-[0_12px_30px_rgba(0,0,0,0.6)] transition-all duration-300 cursor-pointer group transform-gpu"
                            >
                ...
                <div className="w-full max-w-7xl mx-auto mb-20 px-2 sm:px-6">
                    <div className="w-full p-6 sm:p-8 rounded-3xl bg-navy-900/80 backdrop-blur-xl border border-white/15 shadow-[0_15px_45px_rgba(0,0,0,0.5)] relative overflow-hidden">
                        ...
                        {/* Top Tricolor Accent Line */}
                        <div className="absolute top-0 left-0 right-0 h-1" style={{ background: 'linear-gradient(90deg, #FF9933 0%, #FFFFFF 50%, #138808 100%)' }}></div>
